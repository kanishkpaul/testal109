...
7. **Flat Autoregressive Decode Latency, on a Different Protocol**: Our decode measurement diverges from the paper's: we time a single mixer or attention block with `decode_len=64`, where the paper uses 128 and reports ResonatorLM *slower* than attention at 2K (0.40×), crossing over between 4K and 8K. We see ResonatorLM faster throughout, so this is a different measurement rather than a reproduction of theirs. ResonatorLM decode latency is completely flat across context lengths (~0.12 to 0.27 ms/token on Apple Silicon M5), whereas Transformer attention decode latency increases with context (2.07 ms at 2K to 2.58 ms at 8K).
8. **Kernel-Tail Benchmark is Not End-to-End Speedup**: The $440\times - 575\times$ figures compare an $O(T \log T)$ FFT against a chunked $O(T^2)$ time-domain reference, not against an optimized attention kernel. In fairness, the paper's own Table 5 caption already states this "should not be interpreted as an end-to-end model speedup" and its Limitations section repeats it; our criticism is narrower, that the abstract and introduction quote the numbers without carrying that caveat.
9. **Effective Memory Span vs Benchmarked Context**: The paper does not claim 1M context (the string does not occur in its source); its largest stated context is 32K. The finding is that the efficiency claims reach 32K while retention does not. Analysis of the learned half-life distribution ($t_{1/2} \le 2048$ tokens) shows that the remaining signal amplitude at 32K tokens is $0.0015\%$, at 100K is $2.0 \times 10^{-15}$, and at 1M tokens is $10^{-147}$ (absolute numerical zero). Linear resonant modes have no mathematical capacity to preserve token information across 100K or 1M context.
...
Both columns are whole-model figures across all 6 layers. The ResonatorLM column is the **resonant state only**, which is the quantity the paper defines; decoding additionally needs 18.0 KiB of $K=3$ depthwise ring buffer, making the true decode cache 30.0 KiB. That is still constant in $T$, so the scaling conclusion is unchanged, but the ratios below are state-to-KV, not cache-to-cache. Per layer the numbers are 3,968.0 KiB to 63,488.0 KiB of KV cache against a constant 2.0 KiB of recurrent state; the ratios are identical either way.
...
*Basis*: Table 4 times a **single** mixer or attention block, matching the paper's block protocol, whereas Table 3 above is a whole-model figure across 6 layers. The 8.6 GB OOM allocation is likewise a single-layer score matrix. Our `decode_len` is 64 against the paper's 128.
...
| **Significant Long-Context Decode Speedup** | 6.47× at 32K, and 0.40× (ResonatorLM *slower*) at 2K, crossing over between 4K and 8K | 13.1× to 16.6× across 2K-8K on Apple M5; we never observe the sub-1× regime at 2K and did not measure 32K | **Not comparable (protocol divergence)** |
...
| **Efficiency claims reach 32K; retention does not** | Tab. 4-5 benchmark 32K; paper makes no 1M claim | Reported $t_{1/2}=2048$ mode has $1.03\times10^{-147}$ amplitude at 1M; even the $t_{1/2}\approx6{,}931$ architectural ceiling gives only $3.7\times10^{-44}$ | **Unsupported by Linear Physics** |
